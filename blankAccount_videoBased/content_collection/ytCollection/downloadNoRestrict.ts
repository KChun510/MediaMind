...
async function create_metaData(input: { valid_file: string }) {
    ...
    writeMetaData_VidPlusSub(input.valid_file, metaContent ?? "")
}
...
(async function() {
    ...
    fs.readFile('client_secret.json', 'utf8', async function processClientSecrets(err, content) {
        ...
        while (totalVideoTime <= maxVideoTime) {
            try {
                const vidIdRes = await getVideosByKeyWords(oAuthToken, { valid_vids: 10, keywords: "HD Gameplay", videoLicense: "any", results: 10, videoDuration: "long" })
