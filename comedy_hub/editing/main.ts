...
import { create_story_over_single_video, cut_video, delete_video, delete_reddit_cont, segment_clip, segment_clip_alt, segment_clip_twoVids_oneMain, downloadYTVideo, create_twoVids_OneStory, create_srt_over_video, create_twoVids_OneMain, checkFiles } from '../sysCallAPI'
...
function mainVideoCleanUp(videoID: string) {
        delMainVidData(videoID)
        delete_video(videoID)
...
async function twoVids_oneMain() {
        let video1: VIDEO_SQL_SCHEMA = (await selectAllFromMainVideo(10))[0]
        let video1Time = videoTime([video1]);

        let video2: VIDEO_SQL_SCHEMA = (await selectAllFromVideo(10))[0]
        let video2Time = videoTime([video2]);

        if (video1Time <= 0) {
                mainVideoCleanUp(video1.videoID)
                video1 = (await selectAllFromMainVideo(10))[0]
                video1Time = videoTime([video1])
        }
        ...
        try {
                ...
                // Need to cut our video, after clip was made ( No overlapping content )
                cut_video(video1.videoLen ?? '00:00:00', video2.videoID)
                mainVideoCleanUp(video1.videoID)
                console.log(`Clip made! ${video1.videoID}`)
        } catch (e) {
                console.error(`There was a E, while editing video: ${video1.videoID} & ${video2.videoID}\n e code of: ${e} `)
                checkFiles({ fileNames: [video1.videoID, video2.videoID] })
        }
